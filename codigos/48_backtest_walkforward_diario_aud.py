# INVALIDADO / SUPERADO (2026-09-23) - el backtest AUD: ver script 64 (lote A) + 65.
...
# costos en 27/32/36.
#
# Issue #12, paso 3: replica el experimento base de la seccion 9.17 (PPO vs.
# buy-and-hold vs. umbral simple vs. umbral cobre, walk-forward de 5 ventanas
# x 60 dias) mismo esquema que 28_backtest_walkforward_diario.py, pero sobre
# el dataset de AUD/USD (45_generar_dataset_rl_diario_aud.py) en vez de
# USD/CLP. Reusa toda la logica generica de 28 via importlib (posiciones_ppo,
# posiciones_umbral_simple, simular_con_gestion_riesgo,
# simular_buy_and_hold_simple, calcular_metricas - ninguna depende de que el
# activo sea CLP, todas operan sobre el df que se les pasa) y reescribe solo
# lo que SI es especifico de la moneda: la ruta del dataset y las etiquetas
# de los graficos (28 tenia "USD/CLP" hardcodeado en el titulo de un grafico).
...
DATASET_PATH = "../datos/resultados/dataset_entrenamiento_rl_diario_aud.csv"
RESULTADOS_DIR = "../datos/resultados"
NOMBRE_ACTIVO = "AUD/USD"
SUFIJO = "aud"


def posiciones_umbral_cobre_apreciacion(df_test, umbral=0.0):
    # NO reusar kelly_diario_mod.posiciones_umbral_cobre (22) - esa funcion
    # tiene el signo hardcodeado para la convencion cruda de usdclp_long.csv
    # (y=CLP por USD, cobre sube -> y BAJA). El dataset de AUD sale del panel
    # (panel_fx_diario.csv, 23_dataset_multi_par_diario.py), ya normalizado a
    # "USD por 1 unidad de moneda extranjera" (cobre sube -> AUD se aprecia
    # -> y SUBE) - signo opuesto. Confirmado con
    # datos/resultados/panel_fx_correlacion_cobre_por_par.csv: TODAS las
    # correlaciones del panel son positivas (AUD +0.37), al reves de la
    # negativa que asume 22. Bug real encontrado al ver -94% en la primera
    # corrida (sospechosamente peor que el +532% de CLP) - corregido antes
    # de reportar, no despues.
