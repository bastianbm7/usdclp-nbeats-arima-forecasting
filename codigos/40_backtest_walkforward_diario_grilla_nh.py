# INVALIDADO / SUPERADO (2026-09-23) - la grilla N x h: ver script 64 (lote D, subconjunto) + 65.
...
# costos en 27/32/36.
#
# Issue #10: barrido completo de take-profit FIJO por horizonte de forecast
# (h1-h5) cruzado con ventana de holding (N=3,5,7) - 15 combinaciones, cada
# una con su propio agente PPO entrenado en el mismo esquema de walk-forward
# que el resto del proyecto (5 ventanas x 60 dias de calendario).
#
# Motivado por el desglose del experimento de TP adaptativo (36/37): el
# subgrupo de operaciones donde el forecast era consistente hasta h3
# concentraba casi toda la ganancia, mientras que el subgrupo "consistente
# solo hasta h2" perdia plata - sugiere que un horizonte de TP mas lejano
# podria tener valor por si solo. Esta grilla lo mapea directamente en vez
# de inferirlo indirectamente del mecanismo adaptativo.
#
# Las 15 combinaciones comparten el MISMO dataset (39_generar_dataset_rl_diario_h5.py)
# - a diferencia de la comparacion 9.20 vs. TP adaptativo (que no era
# apples-to-apples por la falta de semilla fija en NHITS entre corridas
# distintas), aca todas las filas de la tabla final SI son comparables entre
# si sin ese caveat.
#
# Guardado incremental: la tabla de metricas se reescribe despues de CADA
# combinacion (no solo al final) - con ~15 combinaciones x ~15-17 min cada
# una (~4 horas totales), conviene poder inspeccionar progreso parcial sin
...
def cargar_dataset_h5(path=DATASET_PATH_H5):
    return entorno_diario_mod.cargar_dataset(path)  # reusa retorno_1d/nhits_h1_rel/nhits_h2_rel/macd_rel/rsi_norm/posicion_en_rango tal cual; nhits_h3/h4/h5 viajan intactos (no se usan en el estado, solo en la formula de TP)
...
def calcular_metricas(resultado, dias_holding, horizonte_tp):
    ...
    periodos_por_anio = 252 / dias_holding  # ver nota de anualizacion en 9.20 - comparable entre N distintos
...
if __name__ == "__main__":
    df_completo = cargar_dataset_h5()
    print(f"Dataset diario (hasta h5): {len(df_completo)} dias, {df_completo['ds'].min().date()} a {df_completo['ds'].max().date()}")
    print(f"Grilla: N={VALORES_N} x h={VALORES_H} = {len(VALORES_N) * len(VALORES_H)} combinaciones\n")
    ...
    for N in VALORES_N:
        for h in VALORES_H:
            ...
            resultado.to_csv(f"{RESULTADOS_DIR}/grilla_nh_N{N}_h{h}_operaciones.csv", index=False)

            metricas = calcular_metricas(resultado, N, h)
            filas_metricas.append(metricas)
            pd.DataFrame(filas_metricas).to_csv(f"{RESULTADOS_DIR}/grilla_nh_metricas.csv", index=False)
            ...
            print(f"[{combo_num}/{total_combos}] N={N} h={h}: retorno={metricas['retorno_total_%']:.1f}%, "
                  ...
                  f"({elapsed / 60:.1f} min, acumulado {elapsed_total / 60:.1f} min)")

    print(f"\n=== Grilla completa: {total_combos} combinaciones en {(time.time() - inicio_total) / 60:.1f} min ===")
