...
**[Issue #5](https://github.com/bastianbm7/usdclp-nbeats-arima-forecasting/issues/5) reconstruye el pipeline completo a frecuencia DIARIA** para explotar `copper_ret_1d` (el hallazgo de 9.13-9.14, fuera del alcance semanal de todo lo anterior). Primero se validó que el Sharpe ~4.5 del backtest de Kelly no fuera específico del régimen alcista reciente del cobre: repetido en 3 periodos históricos más viejos (2015-2016, 2017-2018, 2020-2021), el Sharpe se sostiene entre 2.07 y 5.42 en los cuatro periodos — la ventana más vieja (2015-2016) da el Sharpe MÁS ALTO. Con eso validado, se reconstruyó todo el pipeline: dataset walk-forward diario (NHITS con cadencia de refit cada 5 días — refit diario estricto medía ~10.7h, inviable; medido y documentado en el código), entorno Gym nuevo (`27_entorno_trading_rl_diario.py`, sin tocar el semanal) con el mismo risk sizing acotado que el agente semanal, y un agente PPO reentrenado walk-forward. En el camino se encontró y corrigió un bug real (take-profit calculado sobre un precio que no siempre estaba del lado ganador de la posición, verificado con el mismo criterio de "sospechar de números sorprendentes" que el resto del proyecto). Resultado final, walk-forward de 300 días out-of-sample: **a diferencia del agente semanal, el agente diario SÍ encuentra una política rentable** (+397.1%, Sharpe 3.88) — aunque queda por debajo de la señal simple del cobre sola (+532.3%, Sharpe 4.39), que sigue siendo más fuerte que cualquier modelo que la combine con las demás variables. 📄 **[Ver el detalle completo, con gráficos y las limitaciones de esta reconstrucción, en las secciones 9.11-9.17 del paper](reportes/paper.md#911-radar-baseline-validar-la-propuesta-1-de-98-antes-de-construir-nada-septiembre-2026)**.

**[Issue #6](https://github.com/bastianbm7/usdclp-nbeats-arima-forecasting/issues/6) responde si el problema es específico de USD/CLP o genérico**: se entrenó una política PPO **multi-activo** (USD/CLP + USD/MXN + USD/BRL + USD/COP a la vez, mismo motor de riesgo real, estado condicionado por un one-hot del par activo para evitar el pooling ciego que perjudicó a CLP en la sección 9.14) y se comparó contra el agente solo-CLP en el mismo walk-forward de 5 ventanas × 20 semanas. **El resultado es idéntico, hasta el centavo**: el agente multi-activo converge exactamente a la misma política de "no operar nunca" ($100.00, 0 operaciones) que el agente solo-CLP. Un diagnóstico adicional (la misma política evaluada también en el tramo out-of-sample propio de MXN/BRL/COP, no solo CLP) confirma que el atractor de inacción **no es específico de CLP**: 0 operaciones en las 20 combinaciones posibles (4 pares × 5 ventanas). Consistente con la lección de 9.14 sobre pooling: evitar el promedio ciego (one-hot en vez de OLS pooled) hace que sumar datos de otras monedas deje de perjudicar a CLP, pero tampoco ayuda — no hay señal real que extraer de ninguna de las 4 series a esta frecuencia. 📄 **[Ver el detalle completo, con gráficos, en las secciones 9.11-9.18 del paper](reportes/paper.md#911-radar-baseline-validar-la-propuesta-1-de-98-antes-de-construir-nada-septiembre-2026)**.

**[Issue #9](https://github.com/bastianbm7/usdclp-nbeats-arima-forecasting/issues/9) prueba si operar varios días en vez de 1 acerca al agente diario a la señal del cobre**: el agente de 9.17 resuelve cada posición contra el cierre del día siguiente, sin margen para un trailing stop real. Se probó un holding fijo de N días (N=1,2,3,5), reutilizando el trailing stop del agente semanal combinado con la corrección de take-profit inválido del agente diario (un bug real reapareció al intentar reusar esa pieza del agente semanal tal cual, encontrado por el mismo smoke-test que compara N=1 contra 9.17 antes de entrenar nada). **Resultado: alargar el holding NO acerca al agente a la señal del cobre — lo aleja.** La coincidencia de dirección PPO-cobre baja de 72.6% (N=1) a 61.7% (N=5), y la razón Sharpe(PPO)/Sharpe(cobre) cae de forma monótona de 0.88 a 0.51 en los 4 valores de N probados. La hipótesis de esta propuesta queda rechazada por la evidencia. 📄 **[Ver el detalle completo, con gráficos, en la sección 9.20 del paper](reportes/paper.md#920-issue-9-operar-varios-días-reduce-la-divergencia-del-agente-con-la-señal-del-cobre-holding-fijo-de-n-días)**.

**Seguimiento sin Issue propio — take-profit adaptativo**: en vez de un TP fijo en h1, se probó dejar que la trayectoria del forecast NHITS (h1→h2→h3) elija el horizonte objetivo según consistencia. El resultado agregado no mejora sobre TP fijo, pero el desglose por horizonte elegido reveló algo real: las operaciones donde el forecast se sostenía hasta h3 concentraban casi toda la ganancia (+$100.90 de 29 operaciones), mientras que el grupo "consistente solo hasta h2" perdía plata (-$35.84 de 27 operaciones) — dos efectos de signo contrario que se cancelan en el agregado. (De paso, un chequeo de precios de la semana calendario anterior como feature nueva se descartó — la correlación se debilita a la mitad entre 2010-2018 y 2018-2026, a diferencia del cobre, que es estable.) 📄 **[Sección 9.22-9.23 del paper](reportes/paper.md#922-chequeo-rápido-precios-de-la-semana-calendario-anterior-como-feature-descartado)**.
...
**[Issue #13](https://github.com/bastianbm7/usdclp-nbeats-arima-forecasting/issues/13) extiende la línea de cobre-CLP a otros commodities (WTI, oro, platino, soja, hierro) y monedas commodity nuevas (NOK, además de BRL/ZAR ya en el panel)**, en 4 fases. **Fase 0**: los 6 candidatos sobreviven el filtro de liquidez (mismo criterio que descartó litio); se corrigió un bug real donde la limpieza de ticks heredada iba a borrar el colapso genuino del WTI a precio negativo del 20-abril-2020. **Fase 1**: screening de 90 pruebas (3 monedas × 5 commodities × 6 rezagos) con corrección FDR — 10 pares sobreviven, reforzados con bootstrap SPA de Hansen (p=0.0 conjunto); la correlación más fuerte de cada moneda coincide con su commodity de exportación dominante (NOK-petróleo, ZAR-platino, BRL-soja). **Fase 2**: 9 de 10 pares se sostienen con Sharpe positivo en 3 regímenes históricos distintos (2014-2016, 2020, 2022). **Fase 3** (TFT sobre el panel de 14 monedas, atacando el problema de pooling de 9.15): **no cumple el criterio de éxito** — Sharpe -1.65, peor que buy-and-hold; el diagnóstico (RMSE ≈ predecir cero, 47.7% de acierto de dirección) muestra que el problema no es el pooling sino que TFT prediciendo retorno a 1 día no aprende señal, ni pooled ni solo-CLP. **Fase 4** (port en PyTorch de la Variable Selection Network + SharpeLoss del Momentum Transformer, kieranjwood/trading-momentum-transformer): con su configuración default de volatility targeting, retorno +36.6% (no cumple el criterio literal de +83.0%) pero **Sharpe 5.29, el más alto de todo el proyecto**, con solo 2.4% de los días saturados en el límite de apalancamiento (contra ~97% de las demás estrategias Kelly) — escalado a un apalancamiento comparable (3×), supera el criterio (+85.3%) sosteniendo el mejor Sharpe (5.42). 📄 **[Ver el detalle completo de las 4 fases, con todas las tablas y los chequeos de verificación, en las secciones 9.30-9.34 del paper](reportes/paper.md#930-issue-13-commodities-nuevos-petróleo-oro-platino-soja-hierro--nokzarbrl--fase-0-verificación-de-liquidez)**.
...
└── 57_momentum_transformer_port.py  # Issue #13 Fase 4: port en PyTorch de VSN+atencion+SharpeLoss (kieranjwood/trading-momentum-transformer) - Sharpe mas alto del proyecto (5.29-5.42), retorno depende del factor de apalancamiento
